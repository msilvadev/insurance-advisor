...
public class Analysis {
    ...
    public Analysis(@PositiveOrZero(message = "{age.positive}") int age,
                    @PositiveOrZero(message = "{dependents.positive}") int dependents,
                    HouseStatus house,
                    @PositiveOrZero(message = "{income.positive}") int income,
                    @EnumNamePattern(regexp = "SINGLE|MARRIED") MaritalStatus maritalStatus,
                    @Size(min = 3, max = 3, message = "{risk.questions}") int[] riskQuestions,
                    Vehicle vehicle) {
        this.age = age;
